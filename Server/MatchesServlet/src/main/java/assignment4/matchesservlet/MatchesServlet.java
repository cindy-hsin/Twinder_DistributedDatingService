...
public class MatchesServlet extends AbstractGetServlet {
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    String cacheKey = "matches:" + swiperId;
    try (Jedis jedis = jedisPool.getResource()) {
      String cachedMatches = jedis.get(cacheKey);
      if (cachedMatches != null) {
        ...
        this.readMatchesCollection(matchesCollection, swiperId, gson, responseMsg, response, cacheKey, jedis);
      }
    }
  }

  private void readMatchesCollection(MongoCollection<Document> collection, Integer swiperId,
      Gson gson, ResponseMsg responseMsg, HttpServletResponse response, String cacheKey, Jedis jedis)
      throws IOException {
    ...
    if (doc == null) {  // <--> No document matches the _id
      ...
      jedis.setex(cacheKey, REDIS_KEY_EXPIRATION_SECONDS, matchesJson);
    }
  }
}
